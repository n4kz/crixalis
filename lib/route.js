...
Route.prototype.to = function (destination) {
	...
	switch (typeof destination) {
		case 'function':
			this.controller.route(this, destination);
			break;
		default:
...
Route.prototype.match = function (context) {
	...
	/* Method */
	if (this.methods && !this.methods[context.method]) {
		return false;
	}

	/* Host */
	if (this.hosts && !this.hosts[context.host]) {
	...
	/* Pattern match */
	if (this.pattern) {
		if (!this.pattern.test(context.url)) {
			return false;
		}

		if (typeof this.capture === 'object') {
			captures = Object.keys(this.capture);
		}
		...
		for (i = 0, l = captures.length; i < l; i++) {
			k = captures[i];
			context.params[this.capture[k]] = RegExp[k];
		}
	}
