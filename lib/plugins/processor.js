...
/**
 * Processor plugin. Useful for writing preprocessor plugins (like coffee)
 * @module Crixalis
 * @submodule processor
...
function serveCache (filename) {
	...
	this.path = context.cacheDirectory + cache + '.' + this.options.extension;
...
function compile (filename) {
	...
	try {
		result = this.options.compile.call(undefined, filename, this.data, this.options);
	} catch (error) {
		this.forward(error);
		return;
	}
...
module.exports = function (options) {
	...
	Crixalis._(options.method, function (filename) {
